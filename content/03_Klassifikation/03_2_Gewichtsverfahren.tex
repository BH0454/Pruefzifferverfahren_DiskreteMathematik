...
Ein klassisches Beispiel für ein Gewichtsverfahren ist die Berechnung der Prüfziffer der \ac{EAN}, die insbesondere zur eindeutigen Identifikation von Handelsprodukten dient. Die EAN findet sich in der Regel sowohl im Klartext als auch in Form eines Strichcodes auf Produktverpackungen und Etiketten. Die gebräuchliche EAN-13 besteht aus insgesamt 13 Ziffern, wobei die letzte Ziffer eine Prüfziffer darstellt. Diese wird mittels eines Modulo-10-Verfahrens ermittelt, um Eingabefehler (z. B. Zahlendreher oder Einzelfehler) zuverlässig zu erkennen.
...
\end{equation}
Dieses Verfahren gewährleistet eine effiziente Plausibilitätskontrolle der EAN-Nummer im praktischen Einsatz \cite{Hoffmann2023-sv}. Abbildung \ref{fig:ean13} zeigt die Struktur und Berechnung der EAN-13 Prüfziffer.
...
In Anhang \ref{apx:1} ist ein Python-Skript zu finden, mit dem die Prüfziffer einer EAN-13 berechnet werden kann.
...
\end{equation}
Gegeben sei die 14-stellige Kreditkartennummer \cite{Luhn_Beispiel}: \newline \begin{equation*}
...
\hline
\end{tabular}
\caption{Bestimmung der Summanden zur Berechnung der Gesamtsumme S}
\label{tab:Luhn-Beispiel}
\end{table}
...
In Anhang \ref{apx:3} ist ein Python-Skript zu finden, mit dem die Prüfziffer eines Luhn-Algorithmus bestimmt werden kann.