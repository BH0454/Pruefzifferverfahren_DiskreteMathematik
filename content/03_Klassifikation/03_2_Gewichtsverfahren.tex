...
Die ISBN-13, die seit 2007 die ISBN-10 ersetzt hat, bietet gegenüber ihrem Vorgänger mehrere Vor- und Nachteile:

\textbf{Vorteile der ISBN-13:}
\begin{itemize}
    \item \textbf{Größerer Nummernraum:} Die Umstellung auf 13 Stellen ermöglicht eine deutlich größere Anzahl an verfügbaren Kennziffern, was angesichts der steigenden Publikationszahlen notwendig wurde.
    \item \textbf{Kompatibilität:} Die ISBN-13 ist direkt mit dem EAN-System kompatibel, was die Integration in bestehende Logistik- und Handelssysteme erheblich vereinfacht.
    \item \textbf{Keine Sonderzeichen:} Anders als bei ISBN-10 wird kein Sonderzeichen wie 'X' für die Prüfziffer benötigt, was die maschinelle Verarbeitung vereinfacht.
\end{itemize}

\textbf{Nachteile der ISBN-13:}
\begin{itemize}
    \item \textbf{Umstellungsaufwand:} Die Umstellung von ISBN-10 auf ISBN-13 hat erheblichen Aufwand für Verlage, Buchhandel und Bibliotheken verursacht.
    \item \textbf{Fehlererkennungsqualität:} Die ISBN-13 verwendet das Modulo-10-Verfahren mit den alternierenden Gewichten 1 und 3, das im Vergleich zum Modulo-11-Verfahren der ISBN-10 nicht alle Vertauschungsfehler erkennen kann, insbesondere Vertauschungen mit einer Differenz von 5 können unerkannt bleiben.
\end{itemize}
...
Mathematisch lässt sich der Unterschied in der Fehlererkennungsfähigkeit zwischen ISBN-10 und ISBN-13 wie folgt darstellen:

\begin{itemize}
    \item ISBN-10 (Modulo 11): Für Vertauschungsfehler zweier benachbarter Stellen $v_k$ und $v_{k+1}$ gilt:
    \begin{equation}
        (g_k - g_{k+1})(v_{k+1} - v_k) \not\equiv 0 \pmod{11} \quad \forall v_k \neq v_{k+1}
    \end{equation}
    Da $g_k - g_{k+1} = 1$ und $\gcd(1, 11) = 1$ ist, wird jede Vertauschung erkannt.
    
    \item ISBN-13 (Modulo 10): Für Vertauschungsfehler zweier benachbarter Stellen mit alternierendem Gewicht (1 und 3) gilt:
    \begin{equation}
        (1 - 3)(v_{k+1} - v_k) = -2(v_{k+1} - v_k) \not\equiv 0 \pmod{10}
    \end{equation}
    Dies erkennt nur Vertauschungen, bei denen $v_{k+1} - v_k \not\equiv 0 \pmod{5}$ ist. Beispielsweise bleibt eine Vertauschung von 0 und 5 unerkannt, da $-2 \cdot (0 - 5) = 10 \equiv 0 \pmod{10}$.
\end{itemize}
